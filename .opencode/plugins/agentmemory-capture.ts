    ...
    return {
        event: async ({ event }) => {
            ...
            if (type === 'session.created') {
                ...
                activeSessionId =
                    (info?.id as string) || props.sessionID || null
            ...
            if (type === 'session.updated') {
                const info = props.info as Record<string, unknown> | undefined
                const sid =
                    (info?.id as string) || props.sessionID || activeSessionId
                if (!sid) return
            ...
            if (type === 'session.deleted') {
                ...
                if (!sid) {
                    if (DEBUG)
                        console.error(
                            '[agentmemory] session.deleted with no session ID'
                        )
                    return
                }
            ...
            if (type === 'message.updated') {
                ...
                if (info.role === 'assistant') {
                    ...
                    await observe(sid, 'assistant_message', {
                        ...
                        duration_ms:
                            info.time &&
                            typeof (info.time as any).completed === 'number'
            ...
            if (type === 'message.part.updated') {
                ...
                if (part.type === 'step-finish') {
                    await observe(sid, 'step_finish', {
                        ...
                        input_tokens:
                            ((part as any).tokens?.input as number) ?? 0,
                        output_tokens:
                            ((part as any).tokens?.output as number) ?? 0,
                        reasoning_tokens:
                            ((part as any).tokens?.reasoning as number) ?? 0,
                    })
                    return
                }
                ...
                if (part.type === 'file') {
                    const filename =
                        (part as any).filename || (part as any).url || null
